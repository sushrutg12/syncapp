...
-- 1. Add skills column to profiles table as an array
ALTER TABLE "public"."profiles" 
  ADD COLUMN IF NOT EXISTS "skills" text[] DEFAULT '{}';

-- 2. Migrate data from profile_skills junction table to the new column
...
-- 3. First DROP the existing function, then recreate it
DROP FUNCTION IF EXISTS get_profiles(integer);

-- Now recreate with the new return type
...
RETURNS TABLE (
  ...
  skills text[]        -- Use the skills column directly instead of querying it
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 4. Replace the get_profile_skills function to use the new column
DROP FUNCTION IF EXISTS public.get_profile_skills(UUID);
...
AS $$
BEGIN
  RETURN (SELECT skills FROM profiles WHERE id = p_profile_id);
END;
$$;

-- 5. Create a new function to update skills directly
DROP FUNCTION IF EXISTS public.update_profile_skills(UUID, TEXT[]);
...
AS $$
BEGIN
  -- Update the skills array directly in the profiles table
  UPDATE profiles 
  SET skills = p_skill_names
  WHERE id = p_profile_id;
END;
$$;

-- 6. Update the update_candidate_skills function to use the new approach
DROP FUNCTION IF EXISTS public.update_candidate_skills(UUID, TEXT[]);
...
AS $$
BEGIN
  -- Update skills directly in the profiles table
  UPDATE profiles 
  SET skills = p_skill_names
  WHERE id = p_profile_id;
END;
...
GRANT EXECUTE ON FUNCTION public.get_profile_skills(UUID) TO authenticated, anon;
...
-- Keep the skills table for reference but mark it as deprecated
COMMENT ON TABLE "public"."skills" IS 'DEPRECATED - Skills are now stored directly in the profiles table';
COMMENT ON TABLE "public"."profile_skills" IS 'DEPRECATED - Skills are now stored directly in the profiles table';

-- Add popular skills as seed data to make them available for selection
WITH popular_skills AS (
  SELECT 
    unnest(ARRAY[
      'JavaScript', 'TypeScript', 'React', 'React Native', 'Python', 'Java', 'C#', 'Go', 'Ruby', 
      'AWS', 'Firebase', 'Kubernetes', 'PostgreSQL', 'MySQL', 'MongoDB', 'HTML', 'CSS', 
      'PHP', 'Kotlin', 'Swift', 'Angular', 'Vue.js', 'Node.js', 'Photoshop', 'Illustrator', 
      'After Effects', 'Sketch', 'SEO', 'Social Media Management', 'Full-stack developer', 
      'Data Science', 'Machine Learning', 'DevOps', 'UI/UX Design', 'Product Management'
    ]) AS skill_name
)
UPDATE profiles p
SET skills = array_cat(p.skills, ARRAY(
  SELECT ps.skill_name 
  FROM popular_skills ps
  WHERE NOT ps.skill_name = ANY(COALESCE(p.skills, ARRAY[]::text[]))
));
